...
public class CheesMatch {
    ...
    private void undoMove(Position source, Position target, Piece capturedPiece){
        Piece peça = board.removePiece(target);
        board.placePiece(peça, source);
    ...
    private ChessPiece king(Color color){
        ...
        for (Piece peça: list){
            if (peça instanceof King){
                return (ChessPiece) peça;
            }
        }
    ...
    private boolean testCheckMate(Color color){
        ...
        List<Piece> list = piecesOnTheBoard.stream().filter(x ->((ChessPiece)x).getColor() == color).collect(Collectors.toList());
        for (Piece peça : list){
            boolean[][] mat = peça.possibleMoves();
            for (int i = 0; i< board.getRows(); i++){
                for (int j = 0; j<board.getColumns(); j++){
                    if (mat [i][j]){
                        Position source = ((ChessPiece)peça).getChessPosition().toPosition();
